...
- `2026-06-25` - **Production-test run 3, results publication, and diagram-as-code**: ran a third clean end-to-end deploy validating the `make deploy` chaining and the full teardown in production (Word2Vec 87.07%, bit-for-bit identical across three independent deploys); all three live-monitoring methods exercised (watch list-steps, S3 step logs, SSM session). A malformed alert email surfaced a real robustness gap - the apply fails at the SNS subscription while the EMR cluster is already provisioned and billing (independent resources in the Terraform graph) - fixed with plan-time `alert_email` validation (#12, #13). Published measured cost ($0.074/run from Cost Explorer) in `RESULTS.md` (#11), corrected the live-log SSM path to `/mnt/var/log/hadoop/steps/<step-id>/stdout` and recorded the third run (#14), and added the architecture diagram as code with official AWS icons plus a color-coded Mermaid source, refined for cross-theme readability (#15, #16).
- `2026-06-22` - **Deploy validation and post-deploy hardening**: deployed the published repo end to end on AWS twice from a clean clone, first full pipeline runs ever (Word2Vec 87.07% accuracy, bit-for-bit deterministic across both runs); measured cost $0.074/run in Cost Explorer. Five PRs closing the loop: ARM/Graviton migration and EMR bid/service-role fixes found during the real deploys (#5-#7), versioned-bucket teardown with a two-step destroy (#8, ADR 0009), developer-experience and doc reconciliation (#9), and the PT-BR hook refactored to scan git-tracked files (#10). Results published in `RESULTS.md`.
